...
def graph_boxplot(df, columns, num_columns=3, figsize=(14, 10), title="Boxplot"):
...
def graph_correlations(pearson, spearmann, kendall, title, num_columns=1, cmap=['coolwarm', 'viridis', 'plasma'], figsize=(20, 8)):
    methods = [("Pearson Method", pearson), ("Spearmann Method", spearmann), ("Kendall Method", kendall)]
    num_rows = int(np.ceil(len(methods) / num_columns))
    fig, axes = plt.subplots(nrows=num_rows, ncols=num_columns, figsize=figsize, squeeze=False)
    axes = axes.flatten()
    for idx, (method_name, matrix) in enumerate(methods):
        sns.heatmap(
            matrix,
            annot=True,
            cmap=cmap[idx % len(cmap)],
            center=0,
            ax=axes[idx],
        )
